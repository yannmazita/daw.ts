...
import {
  ...
  SequencerTrackInfo,
  SequencerStep,
} from "@/core/interfaces/sequencer";
...
export const useSequencerStore = create<SequencerState>()(
  devtools(
    (set, get) => ({
      ...
      addTrack: (track: SequencerTrackInfo) =>
        set((state) => ({ trackInfo: [...state.trackInfo, track] })),
      updateTrackInfo: (
        update: Partial<SequencerTrackInfo> & { trackIndex: number },
      ) =>
        set((state) => {
          const newTrackInfo = state.trackInfo.map((track) =>
            track.trackIndex === update.trackIndex
              ? { ...track, ...update }
              : track,
          );
      ...
      updateStepsForTrack: (
        ...
        set((state) => {
          const newSteps = state.steps
            .filter((step) => step.trackIndex !== trackIndex)
            .concat(updatedSteps);
          return { steps: newSteps };
        }),
      setStep: (newStep: SequencerStep) =>
        set((state) => {
          const stepIndex = state.steps.findIndex(
            (step) =>
              step.trackIndex === newStep.trackIndex &&
              step.stepIndex === newStep.stepIndex,
          );
          if (stepIndex !== -1) {
            const newSteps = [...state.steps];
      ...
      setSteps: (steps: SequencerStep[]) =>
        set({ steps: steps.map((step) => ({ ...step })) }),
      setCurrentStep: (currentStep: number) => set({ currentStep }),
      toggleStep: (trackIndex: number, stepIndex: number) =>
        set((state) => {
          const trackInfo = state.trackInfo.find(
      ...
      initializeTracks: (numTracks: number) =>
        set((state) => {
      ...
      updateTrackInfoAndSteps: (
        ...
        set((state) => {
          const updatedTrackInfo = state.trackInfo.find(
            (t) => t.trackIndex === trackInfoUpdate.trackIndex,
          );

          if (!updatedTrackInfo) {
            throw new Error(
              `Track with index ${trackInfoUpdate.trackIndex} not found`,
            );
          }
          ...
          if ("loopLength" in trackInfoUpdate) {
            const newSteps = state.steps.map((step) => {
              if (
                trackInfoUpdate.loopLength !== undefined &&
          ...
          if (trackInfoUpdate.timeSignature || trackInfoUpdate.stepDuration) {
            ...
            const currentSteps = state.steps.filter(
              (step) => step.trackIndex === trackInfoUpdate.trackIndex,
            );
              ...
              trackInfo: state.trackInfo.map((track) =>
                track.trackIndex === trackInfoUpdate.trackIndex
                  ? {
                      ...track,
                      ...trackInfoUpdate,
                      stepsPerMeasure: newStepsPerMeasure,
                    }
                  : track,
              ),
              steps: [
